...
func main() {
	...
	// fmt.Println(s1, s2)
	// fmt.Println(s1 == nil)
	// fmt.Println(s2 == nil) // nil是null的意思
	...
	// fmt.Printf("s11: %v  len: %d  cap: %d\n", s11, len(s11), cap(s11))
	// s11 = append(s11, "杭州", "珠海", "成都")
	// fmt.Printf("s11: %v  len: %d  cap: %d\n", s11, len(s11), cap(s11))

	// ss := []string{"武汉", "西安"}
	// s11 = append(s11, ss...) // ... 表示拆开
	// fmt.Printf("s11: %v  len: %d  cap: %d\n", s11, len(s11), cap(s11))
	...
	// m1 := make(map[string]int, 10) // 要估算好该map的容量，避免运行期间再动态扩容
	// m1["理想"] = 18
	// m1["籍无名"] = 35
	...
	// delete(m1, "籍无名")
	// fmt.Println(m1["理想"])
	// fmt.Println(m1 == nil) // 已经开辟了内存，所以不为空
